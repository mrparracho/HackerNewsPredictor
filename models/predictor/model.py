...
class HNFeatureEngineer:
        ...
        for word in words:
            if word in self.word_to_ix:
                ...
                if word_ix < self.embeddings.shape[0]:
                    embedding = self.embeddings[word_ix].cpu().numpy()
    ...
    def create_feature_matrix(self, posts_data):
        ...
        for key in feature_matrices:
            ...
            for idx, item in enumerate(feature_matrices[key]):
                if isinstance(item, np.ndarray):
                    print(f"  {key}[{idx}] shape: {item.shape}, dtype: {item.dtype}")
                elif isinstance(item, (list, tuple)):
                    print(f"  {key}[{idx}] list/tuple length: {len(item)}")
                else:
                    print(f"  {key}[{idx}] type: {type(item)}, value: {item}")
            feature_matrices[key] = np.array(feature_matrices[key])
        
        print(f"Created feature matrix with {len(feature_matrices['scores'])} samples")
